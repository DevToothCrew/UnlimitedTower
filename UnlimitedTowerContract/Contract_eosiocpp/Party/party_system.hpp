...
class cparty_system
    ...
    public:
        ...
        void set_party(account_name _user,uint8_t _party_number,const std::vector<uint32_t> &_party_list)
        {
            ...
            eosio_assert(auth_user_iter->a_hero_list[_party_list[hero_party_location]].h_state == ehero_state::set_travel_party ||
                             auth_user_iter->a_hero_list[_party_list[hero_party_location]].h_state == ehero_state::set_complete,
            ...
            for (uint32_t i = max_monster_slot; i < max_servant_slot; ++i)
            {
                ...
                {
                    uint32_t current_hero_slot = user_party_iter->party_list[_party_number].party_id_list[i];
                    uint32_t change_hero_slot = _party_list[hero_party_location];
                    if(current_hero_slot != change_hero_slot)
                    {
                        auth_user_table.modify(auth_user_iter, owner, [&](auto &change_hero_state) {
                            change_hero_state.a_hero_list[change_hero_slot].h_state = ehero_state::set_travel_party;
                            change_hero_state.a_hero_list[current_hero_slot].h_state = ehero_state::set_complete;
                        });
                    }
                    else
                    {
                ...
                if(_party_list[i] == 0)
                {
                    uint32_t current_servant_index = user_party_iter->party_list[_party_number].party_id_list[i];
                    if(current_servant_index != 0)    //빈공간으로 바꾸는 경우
                    {
                        int l_servant_location = -1;
                        while (left <= right)
                        {
                            mid = (left + right) / 2;
                            if (user_servant_iter->servant_list[mid].s_index == user_party_iter->party_list[_party_number].party_id_list[i])
                            {
                                l_servant_location = mid;
                                break;
                            }
                            else if (user_party_iter->party_list[_party_number].party_id_list[i] > user_servant_iter->servant_list[mid].s_index)
                            {
                                left = mid + 1;
                            }
                            else
                        ...
                        eosio_assert(l_servant_location != -1, "not exist servent id");
                        user_party_table.modify(user_party_iter, owner, [&](auto &add_party_servant) {
                            add_party_servant.party_list[_party_number].party_id_list[i] = _party_list[i];
                        });
                        user_servant_table.modify(user_servant_iter, owner, [&](auto &change_servant_state) {
                            change_servant_state.servant_list[l_servant_location].s_state = eobject_state::on_inventory;
                        });
                    }
                    continue;
                }
                ...
                {
                    mid = (left + right) / 2;
                    if (user_servant_iter->servant_list[mid].s_index == _party_list[i])
                    {
                        l_servant_location = mid;
                        break;
                    }
                    else if (_party_list[i] > user_servant_iter->servant_list[mid].s_index)
                    {
                        left = mid + 1;
                    }
                    else
                ...
                user_servant_table.modify(user_servant_iter, owner, [&](auto &change_servant_state) {
                    change_servant_state.servant_list[l_servant_location].s_state = eobject_state::on_party;
                });
            }
            ...
            for(uint32_t i=0;i<max_monster_slot;++i)
            {
                if (_party_list[i] == 0)
                {
                    uint32_t current_monster_index = user_party_iter->party_list[_party_number].party_id_list[i];
                    if (current_monster_index != 0) //빈공간으로 바꾸는 경우
                    {
                        int l_monster_location = -1;
                        while (left <= right)
                        {
                            mid = (left + right) / 2;
                            if (user_monster_iter->monster_list[mid].m_index == user_party_iter->party_list[_party_number].party_id_list[i])
                            {
                                l_monster_location = mid;
                                break;
                            }
                            else if (user_party_iter->party_list[_party_number].party_id_list[i] > user_monster_iter->monster_list[mid].m_index)
                            {
                                left = mid + 1;
                            }
                            else
                        ...
                        eosio_assert(l_monster_location != -1 ,"not exist monster id");
                        user_party_table.modify(user_party_iter, owner, [&](auto &add_party_monster) {
                            add_party_monster.party_list[_party_number].party_id_list[i] = _party_list[i];
                        });
                        user_monster_table.modify(user_monster_iter, owner, [&](auto &change_monster_state) {
                            change_monster_state.monster_list[l_monster_location].m_state = eobject_state::on_inventory;
                        });
                    }
                    continue;
                }
                ...
                {
                    mid = (left+right)/2;
                    if ( user_monster_iter->monster_list[mid].m_index == _party_list[i])
                    {
                        l_monster_location = mid;
                        break;
                    }
                    else if(_party_list[i] > user_monster_iter->monster_list[mid].m_index)
                    {
                        left = mid + 1;
                    }
                    else
                ...
                user_monster_table.modify(user_monster_iter, owner, [&](auto &change_monster_state) {
                    change_monster_state.monster_list[l_monster_location].m_state = eobject_state::on_party;
                });
            }
        }
