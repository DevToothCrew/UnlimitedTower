                                      ...
                                      uint64_t _sell_item_count)                                     
{
    ...
    if (item_id_iter == equipitem_id_db_table.end())
    {
        equipitem_id_db_table.emplace(_self, [&](auto &new_item_id) {
            ...
            if (_option_value_min != 0)
            {
                new_item_id.option_value_min_list.push_back(_option_value_min);
            }

            if (_option_value_max != 0)
            {
                new_item_id.option_value_max_list.push_back(_option_value_max);
            }

            if (_upgrade_option_value != 0)
            {
                new_item_id.upgrade_option_value_list.push_back(_upgrade_option_value);
            }

            if (_random_option_id != 0)
            {
                new_item_id.random_option_id_list.push_back(_random_option_id);
            }
            if (_grade_multi != 0)
            {
                new_item_id.grade_multi_list.push_back(_grade_multi);
            }
...
void battletest::insert_servant_grind_item(uint64_t _servant_job, uint64_t _item_id)
{
    grinditem_db grinditem_db_table(_self, _self.value);
    auto grinditem_db_iter = grinditem_db_table.find(_servant_job);
    if (grinditem_db_iter == grinditem_db_table.end())
    {
        grinditem_db_table.emplace(_self, [&](auto &new_grind_item) {
            new_grind_item.servant_job = _servant_job;
            new_grind_item.item_id = _item_id;
        });
    }
    else
    {
        grinditem_db_table.modify(grinditem_db_iter, _self, [&](auto new_grind_item) {
            new_grind_item.servant_job = _servant_job;
            new_grind_item.item_id = _item_id;
        });
    }
...
ACTION battletest::dberase(std::string _table, std::string _value)
{
    ...
    // {
    //     value = atoll(_value.c_str());
    //     erase_upgrade_equipment_ratio(value);
    // }

    else if (_table == "dbgrind")
    {
        value = atoi(_value.c_str());
        erase_servant_grind_item(value);
    }
    else if (_table == "dbservant")
    {
        value = atoll(_value.c_str());
    ...
    else if (_table == "dbstatusequi")
    {
        value = atoll(_value.c_str());
        erase_equipment_lv_status(value);
    }
    else if (_table == "dbstage") //스테이지 전체를 날림
    {
        value = atoll(_value.c_str());
        erase_stage(value);
    }
    else if (_table == "dbmonsterlist") //스테이지에 몬스터 리스트만 전체 날림
    {
        value = atoll(_value.c_str());
        erase_stage_monster(value);
    }
    else if (_table == "dbreward") //보상 테이블을 전체 날림
    {
        value = atoll(_value.c_str());
        erase_stage_reward(value);
    }
    else if (_table == "dbrewardlist") //보상 테이블에 보상 리스트만 날림
    {
        value = atoll(_value.c_str());
        erase_reward(value);
    }
...
// void battletest::erase_upgrade_equipment_ratio(uint32_t _main)  //삭제 예정 
// {
//     upgrade_equipment_ratio_db upgrade_equipment_ratio_db_table(_self, _self.value);
//     auto upgrade_equipment_ratio_db_iter = upgrade_equipment_ratio_db_table.find(_main);
//     eosio_assert(upgrade_equipment_ratio_db_iter != upgrade_equipment_ratio_db_table.end(), "Not Exist upgrade Main Item Data");
//     upgrade_equipment_ratio_db_table.erase(upgrade_equipment_ratio_db_iter);
// }

void battletest::erase_servant_grind_item(uint32_t _servant_job)
{
    grinditem_db grinditem_db_table(_self, _self.value);
    auto grinditem_db_iter = grinditem_db_table.find(_servant_job);
    eosio_assert(grinditem_db_iter != grinditem_db_table.end(), "Not Exist girnd Item Data");
    grinditem_db_table.erase(grinditem_db_iter);
}
...
void battletest::erase_stage_monster(uint64_t _id)
{
    stage_db stage_db_table(_self, _self.value);
    auto stage_iter = stage_db_table.find(_id);
    eosio_assert(stage_iter != stage_db_table.end(), "Not Exist Stage 1");
...
void battletest::erase_reward(uint64_t _id)
{
    reward_db reward_db_table(_self, _self.value);
    auto reward_iter = reward_db_table.find(_id);
    ...
    else if (_table == "dbequipup")
    {
        ...
        for (auto iter = upgrade_item_ratio_db_table.begin(); iter != upgrade_item_ratio_db_table.end();)
        {
            ...
            my_table.emplace(_self, [&](auto &new_data) {
                ...
                new_data.upgrade_ratio.insert(new_data.upgrade_ratio.begin(), data.upgrade_ratio.begin(), data.upgrade_ratio.end());
                new_data.material_count.insert(new_data.material_count.begin(), data.material_count.begin(), data.material_count.end());
                new_data.use_UTG.insert(new_data.use_UTG.begin(), data.use_UTG.begin(), data.use_UTG.end());
            });
            iter++;
        }
    }
    ...
    else if (_table == "dbequipment")
    {
        ...
        for (auto iter = equip_item_table.begin(); iter != equip_item_table.end();)
        {
            ...
            my_table.emplace(_self, [&](auto &new_a) {
                ...
                new_a.option_list.insert(new_a.option_list.begin(), data_iter.option_list.begin(), data_iter.option_list.end());
                new_a.option_value_min_list.insert(new_a.option_value_min_list.begin(), data_iter.option_value_min_list.begin(), data_iter.option_value_min_list.end());
                new_a.option_value_max_list.insert(new_a.option_value_max_list.begin(), data_iter.option_value_max_list.begin(), data_iter.option_value_max_list.end());
                new_a.upgrade_option_value_list.insert(new_a.upgrade_option_value_list.begin(), data_iter.upgrade_option_value_list.begin(), data_iter.upgrade_option_value_list.end());
                new_a.random_option_id_list.insert(new_a.random_option_id_list.begin(), data_iter.random_option_id_list.begin(), data_iter.random_option_id_list.end());
                new_a.grade_multi_list.insert(new_a.grade_multi_list.begin(), data_iter.grade_multi_list.begin(), data_iter.grade_multi_list.end());
...
ACTION battletest::mailopen(eosio::name _user, const std::vector<uint64_t> &_mail_index)
{
    ...
    uint64_t _seed = safeseed::get_seed_value(user_value, now());    
    ...
    for (uint32_t i = 0; i < _mail_index.size(); ++i)
    {
        ...
        if (mail_db_iter->mail_type == 1) //서번트
        {
            ...
            user_servant_table.emplace(_self, [&](auto &move_servant) {
                ...
                eosio_assert(servant_db_iter != servant_db_table.end(), "Not exist servant_db_iter1");
        ...
        else if (mail_db_iter->mail_type == 2) //몬스터
        {
            ...
            user_monster_table.emplace(_self, [&](auto &move_monster) {
                ...
                eosio_assert(monster_db_iter != monster_db_table.end(), "Not exist monster_db_iter2");
        ...
        else if (mail_db_iter->mail_type == 3) //아이템
        {
            ...
            user_equipment_table.emplace(_self, [&](auto &move_item) {
                ...
                eosio_assert(equipitem_db_iter != equipitem_db_table.end(), "Not exist equipment_db_iter3");
                allitem_db commonitem_db_table(_self, _self.value);
                auto commonitem_db_iter = commonitem_db_table.find(mail_db_iter->type_id);
                eosio_assert(commonitem_db_iter != commonitem_db_table.end(), "Not exist commonitem_db_iter 3");
        ...
        else if (mail_db_iter->mail_type == 5) //UTS
        {
            std::string temp = "UTS";
            eosio_assert(check_inventory(_user) == true, "your inventory is full");
            uts_db uts_db_table("epochchasert"_n, "epochchasert"_n.value);
            auto uts_db_iter = uts_db_table.find(mail_db_iter->type_id);
            nftexchange(uts_db_iter->owner, uts_db_iter->master, "servant", uts_db_iter->t_idx);
            user_auth_table.modify(user_auth_iter, _self, [&](auto &change_auth_user) {
                change_auth_user.current_servant_inventory += 1;
            });
            action(permission_level{get_self(), "active"_n},
                   NFT_CONTRACT, "deletedata"_n,
                   std::make_tuple(temp, uts_db_iter->t_idx))
                .send();
        }
        ...
        else if (mail_db_iter->mail_type == 6) //UTM
        {
            std::string temp = "UTM";
            eosio_assert(check_inventory(_user) == true, "your inventory is full");
            utm_db utm_db_table("epochchasert"_n, "epochchasert"_n.value);
            auto utm_db_iter = utm_db_table.find(mail_db_iter->type_id);
            nftexchange(utm_db_iter->owner, utm_db_iter->master, "servant", utm_db_iter->t_idx);
            user_auth_table.modify(user_auth_iter, _self, [&](auto &change_auth_user) {
                change_auth_user.current_monster_inventory += 1;
            });
            action(permission_level{get_self(), "active"_n},
                   NFT_CONTRACT, "deletedata"_n,
                   std::make_tuple(temp, utm_db_iter->t_idx))
                .send();
        }
        else if (mail_db_iter->mail_type == 7) //UTI
        {
            std::string temp = "UTI";
            eosio_assert(check_inventory(_user) == true, "your inventory is full");
            uti_db uti_db_table("epochchasert"_n, "epochchasert"_n.value);
            auto uti_db_iter = uti_db_table.find(mail_db_iter->type_id);
            nftexchange(uti_db_iter->owner, uti_db_iter->master, "servant", uti_db_iter->t_idx);
            ...
            action(permission_level{get_self(), "active"_n},
                   NFT_CONTRACT, "deletedata"_n,
                   std::make_tuple(temp, uti_db_iter->t_idx))
                .send();
        }
...
void battletest::eosiotoken_transfer(eosio::name sender, eosio::name receiver, T func)
{
    ...
    else if (res.action =="buyinventory") //인벤토리 구매
    {
        ...
        eosio_assert(transfer_data.memo.find(':') != std::string::npos, "Seed Memo [:] Error");
        eosio_assert(transfer_data.quantity.amount == 05000, "Gacha need 0.5000 EOS");
...
void battletest::gacha_item_id(eosio::name _user, uint64_t _seed)
{
    ...
    user_item_table.emplace(_self, [&](auto &update_user_item_list) {
        ...
        new_item.value = change_equipment_statue((new_item.grade + new_item.type * 10), new_item.value);
...
ACTION battletest::resultgacha(eosio::name _who, eosio::name _type, std::string _result)
{
    require_auth(_self);
...
{
    require_auth(_user);
    eosio_assert(_party_number == 1, "wrong party_number");                                                   //잘못된 파티 넘버 체크
    user_partys user_party_table(_self, _user.value);                                                        //
...
uint32_t battletest::get_magic_attack(status_info _status)
{
    ...
    magic_attack = ((_status.basic_int) * 220) / decimal; //2.2 * 100
    return magic_attack;
}
uint32_t battletest::get_physical_attack(status_info _status)
{
    uint32_t physical_attack;
    physical_attack = ((_status.basic_str + _status.basic_dex) * 140) / decimal; //1.4 * 100
...
void battletest::check_physical_attack_option(eosio::name _user, uint32_t _index, uint32_t &_attack)
{
    ...
    eosio_assert(user_servant_iter != user_servant_table.end(),"Not Exist Servant 100");
    ...
    for(uint32_t i = 0; i < user_servant_iter->servant.equip_slot.size(); ++i)
    {
        ...
        eosio_assert(user_equipment_iter != user_equipment_table.end(),"Not Exist Equipment 100");

        auto db_equipment_iter = equipment_db_table.find(user_equipment_iter->equipment.id);
        eosio_assert(db_equipment_iter != equipment_db_table.end(),"Not Exist Equipment ID 1");
...
void battletest::check_magic_attack_option(eosio::name _user,uint32_t _index, uint32_t &_attack)
{
    user_servants user_servant_table(_self, _user.value);
    auto user_servant_iter = user_servant_table.find(_index);
    eosio_assert(user_servant_iter != user_servant_table.end(),"Not Exist Servant 200");
    ...
    for(uint32_t i = 0; i < user_servant_iter->servant.equip_slot.size(); ++i)
    {
        ...
        auto user_equipment_iter = user_equipment_table.find(user_servant_iter->servant.equip_slot[i]);
        eosio_assert(user_equipment_iter != user_equipment_table.end(),"Not Exist Equipment 200");

        auto db_equipment_iter = equipment_db_table.find(user_equipment_iter->equipment.id);
        eosio_assert(db_equipment_iter != equipment_db_table.end(),"Not Exist Equipment ID 2");
...
void battletest::check_physical_defense_option(eosio::name _user, uint32_t _index, uint32_t &_defense)
{
    user_servants user_servant_table(_self, _user.value);
    auto user_servant_iter = user_servant_table.find(_index);
    eosio_assert(user_servant_iter != user_servant_table.end(),"Not Exist Servant 300");
    ...
    for(uint32_t i = 0; i < user_servant_iter->servant.equip_slot.size(); ++i)
    {
        ...
        auto user_equipment_iter = user_equipment_table.find(user_servant_iter->servant.equip_slot[i]);
        eosio_assert(user_equipment_iter != user_equipment_table.end(),"Not Exist Equipment 300");

        auto db_equipment_iter = equipment_db_table.find(user_equipment_iter->equipment.id);
        eosio_assert(db_equipment_iter != equipment_db_table.end(),"Not Exist Equipment ID 3");
...
void battletest::check_magic_defense_option(eosio::name _user, uint32_t _index, uint32_t &_defense)
{
    user_servants user_servant_table(_self, _user.value);
    auto user_servant_iter = user_servant_table.find(_index);
    eosio_assert(user_servant_iter != user_servant_table.end(),"Not Exist Servant 400");
    ...
    for(uint32_t i = 0; i < user_servant_iter->servant.equip_slot.size(); ++i)
    {
        ...
        auto user_equipment_iter = user_equipment_table.find(user_servant_iter->servant.equip_slot[i]);
        eosio_assert(user_equipment_iter != user_equipment_table.end(),"Not Exist Equipment 400");

        auto db_equipment_iter = equipment_db_table.find(user_equipment_iter->equipment.id);
        eosio_assert(db_equipment_iter != equipment_db_table.end(),"Not Exist Equipment ID 4");
...
void battletest::set_passive_effect(character_state_data &_state, uint32_t &_physical_atk, uint32_t &_physical_dfs, uint32_t &_magic_atk, uint32_t &_magic_defense)
{
    for(uint32_t i = 0; i < _state.passive_skill_list.size(); ++i)
    {
        passive_db passive_db_table(_self, _self.value);
        auto passive_iter = passive_db_table.find(_state.passive_skill_list[i]);
        eosio_assert(passive_iter != passive_db_table.end(),"Not Exist Passive 20");
        if (passive_iter->passive_id == passive_name::passive_physical_attack)
        {
            _physical_atk +=  (_physical_atk * passive_iter->effect_value) / 100;
        }
        else if (passive_iter->passive_id == passive_name::passive_physical_defense)
        {
            _physical_dfs +=  (_physical_dfs * passive_iter->effect_value) / 100;
        }   
        else if (passive_iter->passive_id == passive_name::passive_magic_attack)
        {
            _magic_atk +=  (_magic_atk * passive_iter->effect_value) / 100;
        }
        else if (passive_iter->passive_id == passive_name::passive_magic_defense)
        {
            _magic_defense +=  (_magic_defense * passive_iter->effect_value) / 100;
        }
    }
}
...
battletest::status_info battletest::get_level_up_monster_status(uint64_t _class, uint64_t _grade, status_info _status)
{
    ...
    eosio_assert(monster_lv_db_iter != monster_lv_db_table.end(),"Not Exist Lv ID 1");
...
battletest::status_info battletest::get_level_up_servant_status(uint64_t _job, status_info _status)
{
    ...
    servant_lv_db servant_lv_db_table(_self, _self.value);
    auto servant_lv_db_iter = servant_lv_db_table.find(_job);
    eosio_assert(servant_lv_db_iter != servant_lv_db_table.end(),"Not Exist Lv ID 2");
...
void battletest::set_stage_state(uint64_t _stage_id, std::vector<character_state_data> &_enemy_state_list, std::vector<std::string> &_state)
{
    ...
    for (uint32_t i = 0; i < stage_db_iter->monster_list.size(); ++i)
    {
        ...
        eosio_assert(stage_enemy_db_iter != stage_enemy_db_table.end(), "Not Exist Enemy 1");
        ...
        for (uint32_t i = 0; i < stage_enemy_db_iter->active_list.size(); ++i)
        {
            ...
            eosio_assert(active_db_iter != active_db_table.end(), "Not Exist Enemy Active 1");
        ...
        {
            auto passive_db_iter = passive_db_table.find(stage_enemy_db_iter->active_list[i]);
            eosio_assert(passive_db_iter != passive_db_table.end(), "Not Exist Enemy Passive 1");
...
battletest::character_state_data battletest::get_user_state(eosio::name _user, std::string _type, uint64_t _index, uint32_t _position, std::vector<std::string> &_state)
{
    ...
    if (_type == "ser")
    {
        ...
        eosio_assert(servant_db_iter!= servant_db_table.end(),"Not Exist Servant ID 10");
...
ACTION battletest::stagestart(eosio::name _user, uint32_t _party_number, uint32_t _stage_type, uint32_t _stage_floor)
{
    ...
    system_master system_master_table(_self, _self.value);
    auto system_master_iter = system_master_table.begin();
    eosio_assert(system_master_iter->state != system_state::pause, "Server Pause 11");

    blacklist blacklist_table(_self, _self.value);
    auto blacklist_iter = blacklist_table.find(_user.value);
    eosio_assert(blacklist_iter == blacklist_table.end(), "BlackList User 11");
...
bool battletest::check_activate_skill(uint32_t _skill, uint64_t _rate)
{
    ...
    eosio_assert(active_iter != active_db_table.end(),"Not Exist Active 12");
                            ...
                            std::vector<std::string> &_data)
{
    ...
    else if (_action == action_type::skill)
    {
        ...
        eosio_assert(active_iter != active_db_table.end(),"Not Exist Active 18");
...
void battletest::set_skill_damage(uint32_t _skill_id, uint32_t &_attack, uint32_t &_cri_dmg)
{
    ...
    eosio_assert(active_iter != active_db_table.end(),"Not Exist Active 20");
                                ...
                                uint32_t &_target_defense)
{
    active_db active_db_table(_self, _self.value);
    auto active_iter = active_db_table.find(_skill_id);
    eosio_assert(active_iter != active_db_table.end(),"Not Exist Activ 30");
...
void battletest::set_attack_type(eosio::name _user, uint32_t _atk_type, character_state_data &_state, uint32_t &_attack, uint32_t &_cri_dmg, uint32_t &_cri_per)
{
    ...
    case atk_type::physical_atk:
    {
        if(_state.position < 5)
        {
            ...
            eosio_assert(servant_iter != servant_db_table.end(),"Not Exist Servant ID 40");
...
ACTION battletest::activeturn(eosio::name _user, uint32_t _turn, std::string _seed)
{
    ...
    system_master system_master_table(_self, _self.value);
    auto system_master_iter = system_master_table.begin();
    eosio_assert(system_master_iter->state != system_state::pause, "Server Pause 10");

    blacklist blacklist_table(_self, _self.value);
    auto blacklist_iter = blacklist_table.find(_user.value);
    eosio_assert(blacklist_iter == blacklist_table.end(), "BlackList User 10");
...
{
    monster_db monster_id_db_table(_self, _self.value);
    const auto &monster_id_db_iter = monster_id_db_table.get(_id, "Not Exist Monster ID 2");
...
{
    equipment_db equip_item_table(_self, _self.value);
    const auto &equip_item_iter = equip_item_table.get(_id, "Not Exist Equip ID 1");
...
battletest::item_data battletest::get_reward_item(eosio::name _user, uint32_t _id, uint32_t _count)
{
    ...
    eosio_assert(allitem_db_iter != allitem_db_table.end(), "Not Exist Item 30");
...
void battletest::win_reward(eosio::name _user, uint64_t _stage_number, uint64_t _seed)
{
    ...
    eosio_assert(reward_iter != reward_db_table.end(),"Not Exist Reward 1");
...
///유저 경험치 처리
    ...
    for (uint32_t i = 0; i < reward_iter->reward_list.size(); ++i)
    {
        ...
        {
            uint64_t rate = safeseed::get_random_value(_seed, 1000, 0, 0);
            if (reward_iter->reward_list[i].per >= rate)
            {
                for (uint32_t ser = 0; ser < reward_iter->reward_list[i].count; ++ser)
                {
                    uint64_t seed = _seed;
                    servant_data new_servant = get_reward_servant(_user, reward_iter->reward_list[i].id, reward_iter->reward_list[i].grade, seed);
        ...
        {
            uint64_t rate = safeseed::get_random_value(_seed, 1000, 0, 1);
            if (reward_iter->reward_list[i].per >= rate)
            {
        ...
        {
            uint64_t rate = safeseed::get_random_value(_seed, 1000, 0, 2);
            if (reward_iter->reward_list[i].per >= rate)
            {
        ...
        {
            uint64_t rate = safeseed::get_random_value(_seed, 1000, 0, 4);
            if (reward_iter->reward_list[i].per >= rate)
            {
    ...
    if (user_battle_reward_iter == battle_reward_list_table.end())
    {
        battle_reward_list_table.emplace(_self, [&](auto &set_reward) {
            ...
            set_reward.get_char_exp_list.insert(set_reward.get_char_exp_list.begin(), get_char_exp_list.begin(), get_char_exp_list.end());
            if(servant_list.size() != 0)
            {
                set_reward.get_servant_list.insert(set_reward.get_servant_list.begin(), servant_list.begin(), servant_list.end());
            }
            if(monster_list.size() != 0)
            {
                set_reward.get_monster_list.insert(set_reward.get_monster_list.begin(), monster_list.begin(), monster_list.end());
            }
            if(equipment_list.size() != 0)
            {
                set_reward.get_equipment_list.insert(set_reward.get_equipment_list.begin(), equipment_list.begin(), equipment_list.end());
            }
            if(item_list.size() != 0)
            {
                set_reward.get_item_list.insert(set_reward.get_item_list.begin(), item_list.begin(), item_list.end());
            }
        });
    }
    else
...
ACTION battletest::servantgrind(eosio::name _user, const std::vector<uint64_t> &_servant_list)
{
    require_auth(_user);
    blacklist blacklist_table(_self, _self.value);
    auto blacklist_iter = blacklist_table.find(_user.value);
    ...
    // asset servant_grind_result(0, symbol(symbol_code("UTG"), 4));
    ...
    for (uint32_t i = 0; i < _servant_list.size(); ++i)
    {
        ...
        auto servant_db_iter = servant_db_table.find(user_servant_iter->servant.id);
        auto grinditem_db_iter = grinditem_db_table.find(servant_db_iter->job);
        auto user_items_iter = user_items_table.find(grinditem_db_iter->item_id);
        eosio_assert(user_auth_iter->current_servant_inventory >= 0, "underflow error");

        if (user_items_iter == user_items_table.end())
        {
            if (servant_db_iter->job == 1) //전사
            {
                user_items_table.emplace(_self, [&](auto &change_consumable) {
                    change_consumable.id = 110010;
                    change_consumable.type = 0;
                    change_consumable.count = 1;
                });
            }
            else if (servant_db_iter->job == 2) //도적
            {
                user_items_table.emplace(_self, [&](auto &change_consumable) {
                    change_consumable.id = 110020;
                    change_consumable.type = 0;
                    change_consumable.count = 1;
                });
            }
            else if (servant_db_iter->job == 3) //성직자
            {
                user_items_table.emplace(_self, [&](auto &change_consumable) {
                    change_consumable.id = 110030;
                    change_consumable.type = 0;
                    change_consumable.count = 1;
                });
            }
            else if (servant_db_iter->job == 4) //아처
            {
                user_items_table.emplace(_self, [&](auto &change_consumable) {
                    change_consumable.id = 110040;
                    change_consumable.type = 0;
                    change_consumable.count = 1;
                });
            }
            else if (servant_db_iter->job == 5) //매지션
            {
                user_items_table.emplace(_self, [&](auto &change_consumable) {
                    change_consumable.id = 110050;
                    change_consumable.type = 0;
                    change_consumable.count = 1;
                });
            }
        ...
        //servant_grind_result.amount += 1000000;
    }
    user_auth_table.modify(user_auth_iter, _self, [&](auto &change_auth_user) {
        ...
        change_auth_user.current_item_inventory += _servant_list.size();
    });
    // action(permission_level{get_self(), "active"_n},
    //        get_self(), "transfer"_n,
    //        std::make_tuple(_self, _user, servant_grind_result, std::string("servant grind result")))
    //     .send();
}

ACTION battletest::monstersell(eosio::name _user, const std::vector<uint64_t> &_monster_list)
{
    require_auth(_user);
...
ACTION battletest::equipsell(eosio::name _user, const std::vector<uint64_t> &_equipment_list)
{
    require_auth(_user);
    blacklist blacklist_table(_self, _self.value);
    auto blacklist_iter = blacklist_table.find(_user.value);
    ...
    for (uint32_t i = 0; i < _equipment_list.size(); ++i)
    {
        ...
        eosio_assert(user_equipment_iter != user_equipment_table.end(), "not exist item info");
        eosio_assert(user_equipment_iter->equipment.state != object_state::on_equip_slot, "item is equip");

        equipment_sell_result.amount += 1000000;
...
ACTION battletest::itemsell(eosio::name _user, const std::vector<uint64_t> &_item_list, const std::vector<uint64_t> &_count_list)
{
    require_auth(_user);
    blacklist blacklist_table(_self, _self.value);
    auto blacklist_iter = blacklist_table.find(_user.value);
    ...
    for (uint32_t i = 0; i < _count_list.size(); ++i)
    {
        auto user_item_iter = user_item_table.find(_item_list[i]);
        eosio_assert(user_item_iter != user_item_table.end(), "not exist item info");
        eosio_assert(user_item_iter->count >= _count_list[i], "not sell item count");

        item_sell_result.amount += 1000000 * _count_list[i];

        user_item_table.modify(user_item_iter, _self, [&](auto &change_user_item) {
            change_user_item.count -= _count_list[i];
        });
    }
    ...
    user_auth_table.modify(user_auth_iter, _self, [&](auto &change_auth_user) {
        ...
        {
            change_auth_user.current_item_inventory -= _count_list.size();
        }
    });
}
#pragma endregion
...
ACTION battletest::equip(eosio::name _user, uint32_t _servant_index, uint32_t _item_index)
{
    ...
    eosio_assert(user_servant_iter != user_servant_table.end(), "not exist servant info");
    eosio_assert(user_equip_item_iter != user_equip_item_table.end(), "not exist item info1");
    eosio_assert(user_equip_item_iter->equipment.state != object_state::on_equip_slot, "already equip this item");

    eosio_assert(compare_item(servant_db_iter->job, user_equip_item_iter->equipment.job), "this item is not equipment");

    uint32_t slot;
    if (user_equip_item_iter->equipment.type >= 3)
    {
        slot = 2;
    }
    else
    {
        slot = user_equip_item_iter->equipment.type - 1;
    }
    ...
    if (user_servant_iter->servant.equip_slot[slot] != 0) //기존에 아이템 착용 상태 시
    {
        ...
        //item_status = user_equip_item_iter2->equipment.status;
        ...
        user_servant_table.modify(user_servant_iter, _self, [&](auto &unequip_servant) {
            // unequip_servant.servant.status.plus_str;// += (user_equip_item_iter->equipment.status.basic_str + user_equip_item_iter->equipment.status.plus_str);
            // unequip_servant.servant.status.plus_dex;// += (user_equip_item_iter->equipment.status.basic_dex + user_equip_item_iter->equipment.status.plus_dex);
            // unequip_servant.servant.status.plus_int;// += (user_equip_item_iter->equipment.status.basic_int + user_equip_item_iter->equipment.status.plus_int);
...
ACTION battletest::equipmentup(eosio::name _user, uint32_t _equipment, uint32_t _item)
{
    require_auth(_user);
    blacklist blacklist_table(_self, _self.value);
    auto blacklist_iter = blacklist_table.find(_user.value);
    ...
    equipment_type = user_equipment_iter->equipment.type;
    ...
    eosio_assert(user_equipment_iter != user_equip_item_table.end(), "not exist monster info");
    eosio_assert(user_item_iter != user_item_table.end(), "not exist consumables info");

    eosio_assert(user_equipment_iter->equipment.state == 1, "this item already in Wearing");
    eosio_assert(user_equipment_iter->equipment.upgrade != 9, "this item upgrade is MAX");

    eosio_assert(user_item_iter->id == user_upgrade_equipment_iter->material_id, "upgrade sub item no match");
    eosio_assert(user_item_iter->count >= user_upgrade_equipment_iter->material_count[equipment_upgrade], "not enough material count");
    ...
    user_equip_item_table.modify(user_equipment_iter, _self, [&](auto &upgrade_item) {
        ...
        if (db_ratio >= upgrade_ratio) // 성공 확률
        {
           ...
           // upgrade_item.equipment.status.plus_str += 1;
           // upgrade_item.equipment.status.plus_dex += 1;
           // upgrade_item.equipment.status.plus_int += 1;
            updatecheck = 1;
        }
    });
    user_item_table.modify(user_item_iter, _self, [&](auto &change_consumables) {
        change_consumables.count -= user_upgrade_equipment_iter->material_count[equipment_upgrade];
    });
    user_auth_table.modify(user_auth_iter, _self, [&](auto &change_auth_user) {
        change_auth_user.current_item_inventory -= user_upgrade_equipment_iter->material_count[equipment_upgrade];
    });
...
ACTION battletest::itembuy(eosio::name _user, uint32_t _item_id, uint32_t _count)
{
    ...
    eosio_assert(allitem_db_iter != allitem_db_table.end(), "Not exist 1");
    ...
    if (user_item_iter == user_item_table.end())
    {
        ...
        nomal_order_buy_result.amount = 10000 * _count;
...
void battletest::buy_inventory(eosio::name _user, uint64_t _type)
{
    ...
    {
        user_auth_table.modify(user_auth_iter, _self, [&](auto &change_auth_user) {
            change_auth_user.servant_inventory += 5;
        });
    }
    else if (_type == 2) //몬스터 인벤토리
    {
        user_auth_table.modify(user_auth_iter, _self, [&](auto &change_auth_user) {
            change_auth_user.monster_inventory += 5;
        });
    }
    else if (_type == 3) //장비 인벤토리
    {
        user_auth_table.modify(user_auth_iter, _self, [&](auto &change_auth_user) {
            change_auth_user.equipment_inventory += 5;
        });
    }
    else if (_type == 4) //소모품 인벤토리
    {
        user_auth_table.modify(user_auth_iter, _self, [&](auto &change_auth_user) {
            change_auth_user.item_inventory += 5;
        });
    }
    else
    {
        eosio_assert(_type < 5, "not exsit this action type");
    }
}

ACTION battletest::servantbuy(eosio::name _user, uint32_t _count)
{
    require_auth(_user);
    blacklist blacklist_table(_self, _self.value);
    auto blacklist_iter = blacklist_table.find(_user.value);
...
EOSIO_DISPATCH(battletest,
               (battleaction)(battlestate)(testsnap)(towersnap)(claim)(settower)(deletewhite)(addwhite)
			   (deleteuser)(stageexit)(deletebattle)(saveparty)(stagestart)(activeturn)(setdata)(herocheat)
			   (partycheat)(resultpre)(resultgacha)(create)(issue)(transfer)(setmaster)(settokenlog)(eostransfer)
			   (initmaster)(inittokenlog)(deleteblack)(addblack)(setpause)(dbinsert)(dberase)(dbinit)(alluserdel)
               (servantbuy)(itembuy)(monsterup)(equipmentup)(servantgrind)(changetoken)(resultparty)
               (mailopen)(movedb)(equip)(unequip)(monstersell)(equipsell)(itemsell)(nftmail)(insertequipr))
