...
ACTION battletest::dbinsert(std::string _table, std::string _value)
{
    ...
    if (_table == "dbbuff")
    {
        substr_value(_value, value_list, size_list, 12);
        ...
        if(iter == my_table.end())
        {
            my_table.emplace(_self, [&](auto &new_data) {
                ...
                new_data.effect_stat_give = atoi(value_list[7].c_str());
                new_data.effect_stat_take = atoi(value_list[8].c_str());
                new_data.dmg_type = atoi(value_list[9].c_str());
                new_data.value = atoi(value_list[10].c_str());
                new_data.turn_count = atoi(value_list[11].c_str());
            });
        }
        else
        {
