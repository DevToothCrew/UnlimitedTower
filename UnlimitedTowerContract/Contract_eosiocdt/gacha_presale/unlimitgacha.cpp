...
ACTION unlimitgacha::setdata()
{
    ...
    for (uint8_t i = 0; i < servant_job_count; ++i)
    {
        servant_db_table.emplace(owner, [&](auto &a) {
            uint32_t index = servant_db_table.available_primary_key();
            if (index == 0)
            {
                a.job = START_SERVANT_LOOK;
            }
            else
            {
    ...
    for (uint8_t i = 0; i < monster_grade_count; ++i)
    {
        monster_grade_db_table.emplace(owner, [&](auto &a) {
            uint32_t index = monster_grade_db_table.available_primary_key();
            if (index == 0)
            {
                a.monster_grade = 1;
            }
    ...
    for (uint8_t i = 0; i < monster_id_count; ++i)
    {
        monster_id_db_table.emplace(owner, [&](auto &a) {
            uint32_t index = monster_id_db_table.available_primary_key();
            if(index == 0)
            {
                a.look = START_MONSTER_ID;
            }
            else
            {
    ...
    for (uint8_t i = 0; i < item_id_count; ++i)
    {
        item_id_db_table.emplace(owner, [&](auto &a) {
            uint32_t index = item_id_db_table.available_primary_key();
            if (index == 0)
            {
                a.id = START_ITEM_ID;
            }
            ...
            random_count += 1;
            a.tier = safeseed::get_random_value(l_seed, 6, 1, random_count);
        });
    }
    ...
    for (uint32_t i = 0; i < item_grade_count; ++i)
    {
        item_grade_db_table.emplace(owner, [&](auto &a) {
            uint32_t index = item_grade_db_table.available_primary_key();
            if (index == 0)
            {
                a.grade = 1;
            }
...
void unlimitgacha::presale_gacha_servant_job(eosio::name _user, uint64_t _seed)
{
    servant_db servant_db_table(owner, owner.value);
    uint64_t random_job = safeseed::get_random_value(_seed, servant_job_count + 1, 1, servant_random_count);
    random_job += 10000;
    const auto &servant_db_iter = servant_db_table.get(random_job, "not get servant job data");
    ...
    user_servant_table.emplace(owner, [&](auto &update_user_servant_list) {
        ...
        new_servant.appear.hair = servant_db_iter.job;
        new_servant.appear.head = servant_db_iter.job;
        new_servant.appear.body = servant_db_iter.job;
...
void unlimitgacha::presale_gacha(eosio::name _user, uint64_t _seed)
{
    ...
    if (l_gacha_result_type < 333)
    {
        presale_gacha_servant_job(_user, l_seed);
    }
    else if (l_gacha_result_type > 333 && l_gacha_result_type <= 666)
    {
...
ACTION unlimitgacha::dbinsert(uint32_t _kind, uint32_t _appear, uint32_t _id, uint32_t _job, uint32_t _tier, uint32_t _type, uint32_t _grade, uint32_t _min, uint32_t _max)
{
  require_auth(owner);
  ...
  switch(_kind){
     ...
     case db_choice::gender:
      ...
      {
          insert_item_grade(_grade,min,max);
          break;
      }
...
void unlimitgacha::insert_job(uint32_t _job)
{
...
void unlimitgacha::insert_servant_id(uint32_t _id)
{
...
void unlimitgacha::insert_monster_grade(uint32_t _grade, uint32_t _min, uint32_t _max)
{
    ...
    monster_grade_db_table.emplace(owner, [&](auto &new_monster_grade)
    {
        ...
        new_monster_grade.min_range.basic_dex = _min;
        new_monster_grade.min_range.basic_int = _min;
        new_monster_grade.min_range.basic_str = _min;
        new_monster_grade.max_range.basic_dex = _max;
        new_monster_grade.max_range.basic_int = _max;
        new_monster_grade.max_range.basic_str = _max;
    })
}

void unlimitgacha::insert_item_id(uint32_t id, uint32_t type, uint32_t _job, uint32_t tier)
{
    item_id_db item_id_db_table(owner, owner.value);
...
void unlimitgacha::insert_item_grade(uint32_t _grade, uint32_t _min, uint32_t _max)
{
    ...
    item_grade_db_table.emplace(owner, [&](auto &new_item_grade)
    {
        ...
        new_item_grade.min_range.basic_dex = _min;
        new_item_grade.min_range.basic_int = _min;
        new_item_grade.min_range.basic_str = _min;
        new_item_grade.max_range.basic_dex = _max;
        new_item_grade.max_range.basic_int = _max;
        new_item_grade.max_range.basic_str = _max;
    })
}


ACTION unlimitgacha::dbmodify(uint32_t _kind, uint32_t _appear, uint32_t _id, uint32_t _job, uint32_t _tier, uint32_t _type, uint32_t _grade, uint32_t _min, uint32_t _max)
{
  require_auth(owner);
  ...
  switch(_kind){
     ...
     case db_choice::gender:
      ...
      case db_choice::item_grade:
      {
          modify_item_grade(_grade,min,max);
          break;
      }
...
ACTION unlimitgacha::dbdelete(uint32_t _kind, uint32_t _appear, uint32_t _id, uint32_t _job, uint32_t _tier, uint32_t _type, uint32_t _grade, uint32_t _min, uint32_t _max)
{
  require_auth(owner);
  ...
  switch(_kind){
     ...
     case db_choice::gender:
      {
          delete_gender(_appear);
          break;
      }
      case db_choice::servant_id:
      {
          delete_servant_id(_id);
          break;
      }
      case db_choice::monster_id:
      {
          delete_monster_id(_id);
          break;
      }
      case db_choice::monster_grade:
      {
          delete_monster_grade(_grade,_min,_max);
          break;
      }
      case db_choice::item_id:
      {
          delete_item_id(_id,_type,_job,_tier);
          break;
      }
      case db_choice::item_grade:
      {
          delete_item_grade(_grade,min,max);
          break;
      }
...
void unlimitgacha::delete_body(uint32_t _appear)
{
   body_db body_db_table(owner, owner.value);
   auto body_iter = body_db_table.find(_appear);
   eosio_assert(body_iter != body_db_table.end(),"not find body data");
...
EOSIO_DISPATCH(unlimitgacha, (create)(issue)(tokentrans)(setdata)(setpresale)(presalemove)(eostransfer)(initdata)(deleteuser)(initalluser)(initprelog)(inittoken)(gachacheat)(deleteblack)(addblack)(setpause) )
