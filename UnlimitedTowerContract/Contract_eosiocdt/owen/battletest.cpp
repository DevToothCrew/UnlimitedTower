        ...
        if (system_master_iter == system_master_table.end())    //보낸 계정이 마스터 계정이 아닌경우
        {
            ...
            else if (res.action == "shopbuyitem")
            {
                ...
                else if (res.type == 16 || res.type == 17 || res.type == 18 || res.type == 19 || res.type == 20 || res.type == 21 || res.type == 22 || res.type == 23)
                {
                    ...
                    //eosio_assert(res.count * package_shop_iter == transfer_data.quantity.amount, "Eos Transfer Package Shop : Not same EOS");
                }
                else
                {
...
ACTION battletest::goldgacha(eosio::name _user, string _memo)
{
    ...
    if (action == "goldgacha")
    {
        ...
        gacha_use_UTG_result.amount = 1000 * UTG;
        //gacha_use_UTG_result.amount = UTG;
...
void battletest::gold_gacha(eosio::name _user, uint64_t _seed, uint32_t _second_seed)
{
    ...
    user_log_table.modify(user_log_iter, _self, [&](auto &new_data) {
       // new_data.use_utg += 1000 * UTG;
        new_data.use_utg += UTG;
    });
...
void battletest::gold_gacha_item_id(eosio::name _user, uint64_t _seed)
{
    ...
    gold_logs_table.modify(gold_logs_iter, _self, [&](auto &update_log) {
        ...
        update_log.use_utg += UTG;
        //update_log.use_utg += 1000 * UTG;
    });
}
...
ACTION battletest::itemburn(eosio::name _user, const std::vector<uint64_t> &_sell_item_list, const std::vector<uint64_t> &_sell_count_list)
{
    system_check(_user);
    ...
    uint32_t sell_items_list_count = _sell_item_list.size();

    //팔려는 아이템 리스트 사이즈 만큼 
    for (uint32_t i = 0; i < sell_items_list_count; ++i)
    {
        auto user_item_iter = user_item_table.find(_sell_item_list[i]);
        eosio_assert(user_item_iter != user_item_table.end(), "item burn : not exist item info by user");
        auto allitem_db_iter = allitem_db_table.find(user_item_iter->id);
        eosio_assert(allitem_db_iter != allitem_db_table.end(), "item burn : Not exist allitem info");
  
        //판매 하려는 아이템의 갯수 
        uint64_t sell_item_count = _sell_count_list[i];

        uint64_t current_total_item_count =0;

        uint64_t target_item_count = user_item_iter->item_list.size();

        for (uint64_t j = 0; j < user_item_iter->item_list.size(); j++) 
        {
            current_total_item_count += user_item_iter->item_list[j].count;
        }
        eosio_assert(current_total_item_count >= target_item_count, "itemburn : not enough sell item count");

        eosio_assert(_sell_item_list.size() == _sell_count_list.size(), "itemburn : The lengths of the two do not match");

        if (current_total_item_count == target_item_count)
        {
        ...
        else
        {
            user_item_table.modify(user_item_iter, _self, [&](auto &change_user_item) {
                ...
                for (uint64_t j = change_user_item.item_list.size() - 1; j >= 0; j--)
                {
                    if (change_user_item.item_list[insert_job_stat].count < temp_count)
                    {
                        temp_count = (temp_count - change_user_item.item_list[j].count);
                        change_user_item.item_list[j].count = 0;
                        change_user_item.item_list.erase(sub_item_iter);
                        check_inventory += 1;
                    }
                    else
                    {
                        change_user_item.item_list[j].count -= temp_count;
                        if (change_user_item.item_list[j].count == 0)
                        {
                            change_user_item.item_list.erase(sub_item_iter);
                            check_inventory += 1;
                            sub_item_iter--;
        ...
        item_burn_result.amount += allitem_db_iter->sell_item_count * _sell_count_list[i];
    }
    ...
    user_auth_table.modify(user_auth_iter, _self, [&](auto &change_auth_user) {
        eosio_assert(change_auth_user.current_item_inventory >= _sell_count_list.size(), "itemburn : The current number of item and the number of units to sell are not correct.");
        if (change_auth_user.current_item_inventory - _sell_count_list.size() <= 0)
        {
...
EOSIO_DISPATCH(battletest,
              ...
              (transfer)(changetoken)(create)(issue)            //(inittokenlog) (initmaster)
              //contants
              (goldgacha)(itembuy)(monsterup)(equipmentup)(mailopen)(equip)(nftmail)(burn)      //(itemburn)
              //battle
              (activeturn)(stagestart)(stageexit)(pvpstart)(saveparty)  
              //tower
              (claim)(endflag)(toweropen)(towerstart)(deletetower)(chat)
              //cheat       
              (leveltest)(accountset)(updatecheack)       // (monstercheat)(anothercheck)                                                                                                       
