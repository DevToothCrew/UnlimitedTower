...
uint64_t check_seed(std::string _seed, std::string _sha_result)
{
      ...
      eosio_assert(hexstr == client_sha_seed ,"not same sha256 key");
...
std::vector<uint64_t>& get_total_rand(std::vector<uint64_t> &_array, uint32_t _total_value)
{
     checksum256 result;
     uint64_t mixedBlock = tapos_block_prefix() * tapos_block_num();
