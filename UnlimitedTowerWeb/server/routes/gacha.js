                ...
                if(err){
                    ...
                    connection.query(sql, gachaLogData, function(err){
                        if(err){
                            ...
                            console.log(config.color.green, 'user : ', user, ', func : ', func + " GachaLog DB Insert", ', time : ', new Date(new Date().toUTCString()));
                        }
                    });
                    ...
                    connection.query(sql, eosLogData, function(err){
                        ...
                        if(err){
                            ...
                            console.log(config.color.green, 'user : ', user, ', func : ', func + " EosLog DB Insert", ', time : ', new Date(new Date().toUTCString()));
                        }
                    });
                }
            });
                    ...
                    poolCluster.getConnection(function(err, connection){
                        if(err){
                            ...
                            if(result.result_type == 1){
                                type = "servant";
                                type_index = result.data.servant.id;
                                grade = result.data.servant.grade;
                            }
                            if(result.result_type == 2){
                                type = "monster";
                                grade = result.data.monster.grade;
